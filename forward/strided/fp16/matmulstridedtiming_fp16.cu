...
void gpu_blas_mmul(__half *A, __half *B, __half *C) {
    __half alf =__half(1.0f);
    __half bet =__half( 0.0f);
    __half *alpha = &alf;
    __half *beta = &bet;
...
//			cublasGemmStridedBatchedEx(handle, CUBLAS_OP_N, CUBLAS_OP_N, seq_len, seq_len, size_per_head, alpha, B, CUDA_R_16F, seq_len, seq_len * size_per_head, A, CUDA_R_16F, size_per_head, seq_len * size_per_head, beta, C, CUDA_R_16F, seq_len, seq_len * seq_len, num_heads * batch_size, CUDA_R_32F, CUBLAS_GEMM_DEFAULT_TENSOR_OP );
			

			//for second gemm
			cublasGemmStridedBatchedEx(handle, CUBLAS_OP_N, CUBLAS_OP_N, size_per_head, seq_len, seq_len, alpha, B, CUDA_R_16F, size_per_head, seq_len * size_per_head, A, CUDA_R_16F, seq_len, seq_len * seq_len, beta, C, CUDA_R_16F, size_per_head, seq_len * size_per_head, num_heads * batch_size, CUDA_R_16F, CUBLAS_GEMM_DEFAULT_TENSOR_OP );
}
...
int main() {
    int nr_rows_A, nr_cols_A, nr_rows_B, nr_cols_B, nr_rows_C, nr_cols_C, A_size, B_size, C_size;
		...
		//B_size =  A_size;
		//C_size = batch_size * num_heads * seq_len * seq_len; 	 
		A_size =  batch_size * num_heads * seq_len * seq_len;
		...
		nr_cols_A = seq_len;
		nr_rows_B = seq_len;
		nr_cols_B =	size_per_head;
		nr_rows_C = seq_len;
		nr_cols_C = size_per_head;
