...
void gpu_blas_mmul(__half *A, __half *B, float *C, int m, int k, int n, int iter) {
  ...
  for(int i = 0; i < 5; i++){
    ...
    check_cuda_error(cublasGemmEx(handle, CUBLAS_OP_T, CUBLAS_OP_N, n, m, k, alpha, B, CUDA_R_16F, ldb, A, CUDA_R_16F, lda, beta, C, CUDA_R_32F, ldc, CUBLAS_COMPUTE_32F, CUBLAS_GEMM_DEFAULT_TENSOR_OP));
  }
  //-------------------------------------perform actual runs--------------------------------//
  ...
  std::cout<<m<<", "<<n<<", "<<k<<", "<<teraFlops<<std::endl;
  /*
     printf(
...
int main(int argc, char * argv[]) {
  ...
  int n = atoi(nc);
  int niter = atoi(iter); 
  // printf("forward matmul: m:%d, n:%d, k:%d\n", m, n, k);
  ...
  gpu_blas_mmul(d_A, d_B, df_C, nr_rows_A, nr_cols_A, nr_rows_B, niter);
