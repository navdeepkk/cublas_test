...
void gpu_blas_mmul(__half *A, __half *B, __half *C, int m, int k, int n, int iter) {
	...
	check_cuda_error(cublasGemmEx(handle, CUBLAS_OP_N, CUBLAS_OP_T, n, m, k, alpha, B, CUDA_R_16F, ldb, A, CUDA_R_16F, lda, beta, C, CUDA_R_16F, ldc, CUDA_R_16F, CUBLAS_GEMM_DEFAULT_TENSOR_OP));
}
		//-------------------------------------perform actual runs--------------------------------//
...
int main(int argc, char* argv[]) {
    int nr_rows_A, nr_cols_A, nr_rows_B, nr_cols_B, nr_rows_C, nr_cols_C;
		...
		//B is for the activations. has dimensions k * n. n is the number of inputs in the layer.  
		//C is the output matrix, gradients of the weight. has dimensions m * n.
		//Matmul will be A' B.
		...
		nr_rows_C = m;				//m
		nr_cols_C = n;				//n
		printf("weight gradient(fp16): m:%d, n:%d, k:%d\n", m, n, k);
