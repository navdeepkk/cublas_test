...
void gpu_blas_mmul(__half *A, __half *B, __half *C, int m, int k, int n, int iter) {
    ...
    cublasHandle_t handle;
		...
		float minMatmulTime = 100.0f;
		//-------------------------------peforming warmup runs-------------------------------------//
		for(int i = 0; i < 0; i++){
			// Do the actual multiplication
	check_cuda_error(cublasGemmEx(handle, CUBLAS_OP_N, CUBLAS_OP_N, n, k, m, alpha, B, CUDA_R_16F, ldb, A, CUDA_R_16F, lda, beta, C, CUDA_R_16F, ldc, CUDA_R_16F, CUBLAS_GEMM_DEFAULT_TENSOR_OP));
}
		//-------------------------------------perform actual runs--------------------------------//
	...
	check_cuda_error(cublasGemmEx(handle, CUBLAS_OP_N, CUBLAS_OP_N, n, k, m, alpha, B, CUDA_R_16F, ldb, A, CUDA_R_16F, lda, beta, C, CUDA_R_16F, ldc, CUDA_R_16F, CUBLAS_GEMM_DEFAULT_TENSOR_OP));
		...
		if(msecTotal < minMatmulTime){
			minMatmulTime = msecTotal;	
		}
...
*/	
		...
		double teraFlops = (flopsPerMatrixMul * 1.0e-12f) / (minMatmulTime / 1000.0f);
		printf(
...
int main(int argc, char* argv[]) {
    int nr_rows_A, nr_cols_A, nr_rows_B, nr_cols_B, nr_rows_C, nr_cols_C;
		...
		//A is for the dY. has dimensions k * m, where k is (seq length * batchsize),  m is no of Outputs in the layer. 
		//B is for the weights. has dimensions m * n. n is the number of inputs in the layer.  
		//C is the output matrix, gradients of the activations. has dimensions m * n.
		//Matmul will be A B.
		...
		int n = atoi(nc);
		int niter = atoi(iter); 
		printf("activation gradient(fp16): m:%d, n:%d, k:%d\n", m, n, k);
		nr_rows_A = k;				//k
		nr_cols_A = m;				//m
		nr_rows_B = m;				//m		
		nr_cols_B = n;				//n
    ...
    check_cuda_error(cudaMalloc(&df_C,nr_rows_C * nr_cols_C * sizeof(float)));
	  ...
	  gpu_blas_mmul(d_A, d_B, d_C, nr_cols_A, nr_rows_A, nr_cols_B, niter);
