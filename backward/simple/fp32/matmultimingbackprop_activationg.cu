...
void gpu_blas_mmul(__half *A, __half *B, __half *C, int m, int k, int n, int iter) {
	...
	check_cuda_error(cublasGemmEx(handle, CUBLAS_OP_N, CUBLAS_OP_N, n, k, m, alpha, B, CUDA_R_16F, ldb, A, CUDA_R_16F, lda, beta, C, CUDA_R_16F, ldc, CUDA_R_32F, CUBLAS_GEMM_DEFAULT_TENSOR_OP));
}
		//-------------------------------------perform actual runs--------------------------------//
...
int main(int argc, char * argv[]) {
    int nr_rows_A, nr_cols_A, nr_rows_B, nr_cols_B, nr_rows_C, nr_cols_C;
		...
		int n = atoi(nc);
		int niter = atoi(iter); 
		printf("activation gradient: m:%d, n:%d, k:%d\n", m, n, k);
		nr_rows_A = k;				//k
		nr_cols_A = m;				//m
		nr_rows_B = m;				//m		
		nr_cols_B = n;				//n
