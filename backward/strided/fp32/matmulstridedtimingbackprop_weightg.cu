...
void gpu_blas_mmul(__half *A, __half *B, __half *C) {
    ...
    cublasHandle_t handle;
		...
		for(int i = 0; i < 1; i++){
			...
			cublasGemmStridedBatchedEx(handle, CUBLAS_OP_N, CUBLAS_OP_T, size_per_head, seq_len, seq_len, alpha, B, CUDA_R_16F, size_per_head, seq_len * size_per_head, A, CUDA_R_16F, seq_len, seq_len * seq_len, beta, C, CUDA_R_16F, seq_len, seq_len * size_per_head, num_heads * batch_size, CUDA_R_32F, CUBLAS_GEMM_DEFAULT_TENSOR_OP );
}
